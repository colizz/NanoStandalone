/*
 * AK8 fatjet H->tautau ntuplizer for MiniAOD->NanoAOD custom tagger production.
 *
 * Purpose:
 *   Build a compact event-level ntuple for the fatjet-based boosted H->tautau
 *   reconstruction strategy after producing NanoAOD from MiniAOD with
 *   additional GloParT v1/v2 inference. Each selected event contributes at
 *   most one AK8 candidate, plus common event, reco-summary, generator-truth,
 *   standard ParticleNet/GlobalParT3 payload, ParticleNet raw H->tautau scores,
 *   and GloParT v1/v2 tau-tau scores.
 *
 * Input and output:
 *   - Reads custom NanoAOD Events and Runs trees. Plain /store paths are resolved
 ...
 *     corrections, ParticleNet discriminants/raw Htt/Htm/Hte scores,
 *     GloParT v1/v2 Htauhtaue/Htauhtaum/Htauhtauh scores, and GloParT v1/v2
 *     summed QCD scores.
 *
 * Notes:
...
struct FatJetBranches {
    ...
    TTreeReaderArray<Float_t> particleNet_Htt, particleNet_Htm, particleNet_Hte;
    TTreeReaderArray<Float_t> inclParTMDV1_Htauhtaue, inclParTMDV1_Htauhtaum, inclParTMDV1_Htauhtauh;
    TTreeReaderArray<Float_t> inclParTMDV1_QCDbb, inclParTMDV1_QCDcc, inclParTMDV1_QCDb, inclParTMDV1_QCDc, inclParTMDV1_QCDothers;
    TTreeReaderArray<Float_t> globalParT2_Htauhtaue, globalParT2_Htauhtaum, globalParT2_Htauhtauh;
    TTreeReaderArray<Float_t> globalParT2_QCDbb, globalParT2_QCDcc, globalParT2_QCDb, globalParT2_QCDc, globalParT2_QCDothers;
    ...
    FatJetBranches(TTreeReader& r)
        : nFatJet(r, "nFatJet"), pt(r, "FatJet_pt"), eta(r, "FatJet_eta"), phi(r, "FatJet_phi"),
          ...
          particleNet_Htt(r, "FatJet_particleNet_Htt"),
          particleNet_Htm(r, "FatJet_particleNet_Htm"),
          particleNet_Hte(r, "FatJet_particleNet_Hte"),
          inclParTMDV1_Htauhtaue(r, "FatJet_inclParTMDV1_probHtauhtaue"),
          inclParTMDV1_Htauhtaum(r, "FatJet_inclParTMDV1_probHtauhtaum"),
          inclParTMDV1_Htauhtauh(r, "FatJet_inclParTMDV1_probHtauhtauh"),
          inclParTMDV1_QCDbb(r, "FatJet_inclParTMDV1_probQCDbb"),
          inclParTMDV1_QCDcc(r, "FatJet_inclParTMDV1_probQCDcc"),
          inclParTMDV1_QCDb(r, "FatJet_inclParTMDV1_probQCDb"),
          inclParTMDV1_QCDc(r, "FatJet_inclParTMDV1_probQCDc"),
          inclParTMDV1_QCDothers(r, "FatJet_inclParTMDV1_probQCDothers"),
          globalParT2_Htauhtaue(r, "FatJet_globalParT2_probHtauhtaue"),
          globalParT2_Htauhtaum(r, "FatJet_globalParT2_probHtauhtaum"),
          globalParT2_Htauhtauh(r, "FatJet_globalParT2_probHtauhtauh"),
          globalParT2_QCDbb(r, "FatJet_globalParT2_probQCDbb"),
          globalParT2_QCDcc(r, "FatJet_globalParT2_probQCDcc"),
          globalParT2_QCDb(r, "FatJet_globalParT2_probQCDb"),
          globalParT2_QCDc(r, "FatJet_globalParT2_probQCDc"),
          globalParT2_QCDothers(r, "FatJet_globalParT2_probQCDothers") {}
};
...
void nanoAOD_to_htautau_fatjet_custom_tagger(const char* inputFile, const char* outputFile, const char* object_selection_rule = "max_pt") {
    ...
    Float_t pn_htt = kMissing, pn_htm = kMissing, pn_hte = kMissing;
    Float_t gp1_xtauhe = kMissing, gp1_xtauhm = kMissing, gp1_xtauhh = kMissing, gp1_qcd = kMissing;
    Float_t gp2_xtauhe = kMissing, gp2_xtauhm = kMissing, gp2_xtauhh = kMissing, gp2_qcd = kMissing;
    ...
    out->Branch("FatJet_particleNet_Htt", &pn_htt);
    out->Branch("FatJet_particleNet_Htm", &pn_htm);
    out->Branch("FatJet_particleNet_Hte", &pn_hte);
    out->Branch("FatJet_inclParTMDV1_probHtauhtaue", &gp1_xtauhe);
    out->Branch("FatJet_inclParTMDV1_probHtauhtaum", &gp1_xtauhm);
    out->Branch("FatJet_inclParTMDV1_probHtauhtauh", &gp1_xtauhh);
    out->Branch("FatJet_inclParTMDV1_QCD", &gp1_qcd);
    out->Branch("FatJet_globalParT2_probHtauhtaue", &gp2_xtauhe);
    out->Branch("FatJet_globalParT2_probHtauhtaum", &gp2_xtauhm);
    out->Branch("FatJet_globalParT2_probHtauhtauh", &gp2_xtauhh);
    out->Branch("FatJet_globalParT2_QCD", &gp2_qcd);
    ...
    while (reader.Next()) {
        ...
        pn_htt = pn_htm = pn_hte = kMissing;
        gp1_xtauhe = gp1_xtauhm = gp1_xtauhh = gp1_qcd = kMissing;
        gp2_xtauhe = gp2_xtauhm = gp2_xtauhh = gp2_qcd = kMissing;
        ...
        if (lead >= 0) {
            ...
            pn_htt = fat.particleNet_Htt[lead];
            pn_htm = fat.particleNet_Htm[lead];
            pn_hte = fat.particleNet_Hte[lead];
            gp1_xtauhe = fat.inclParTMDV1_Htauhtaue[lead];
            gp1_xtauhm = fat.inclParTMDV1_Htauhtaum[lead];
            gp1_xtauhh = fat.inclParTMDV1_Htauhtauh[lead];
            gp1_qcd = fat.inclParTMDV1_QCDbb[lead] + fat.inclParTMDV1_QCDcc[lead] +
                      fat.inclParTMDV1_QCDb[lead] + fat.inclParTMDV1_QCDc[lead] +
                      fat.inclParTMDV1_QCDothers[lead];
            gp2_xtauhe = fat.globalParT2_Htauhtaue[lead];
            gp2_xtauhm = fat.globalParT2_Htauhtaum[lead];
            gp2_xtauhh = fat.globalParT2_Htauhtauh[lead];
            gp2_qcd = fat.globalParT2_QCDbb[lead] + fat.globalParT2_QCDcc[lead] +
                      fat.globalParT2_QCDb[lead] + fat.globalParT2_QCDc[lead] +
                      fat.globalParT2_QCDothers[lead];
        }
